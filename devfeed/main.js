...
buscaInput?.addEventListener('input', debounce(e => store.setState({ termoBusca: e.target.value }), 300))
lista?.addEventListener('click', e => {
  const tagEl = e.target.closest('.tag[data-tag]')
  if (tagEl) { const { filtroTag } = store.getState(); store.setState({ filtroTag: filtroTag === tagEl.dataset.tag ? null : tagEl.dataset.tag }); return }
  const btn = e.target.closest('.btn-remover[data-id]')
  if (btn) { const { links } = store.getState(); store.setState({ links: removeLink(links, Number(btn.dataset.id)) }) }
})
...
btnLimparFiltro?.addEventListener('click', () => { store.setState({ filtroTag: null, termoBusca: '' }); if (buscaInput) buscaInput.value = '' })
...
store.setState({ links: linksSalvos.length > 0 ? linksSalvos : [
